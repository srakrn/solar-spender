...
var SolarSpenderPanelHost = class extends HTMLElement {
  ...
  _fillStandardFields() {
    ...
    ["enabled", "source_type", "grid_export_positive", "battery_policy"].forEach((key) => {
      form.elements[key].value = String(this._options[key]);
    });
    ...
    this._options.loads.forEach((load, index) => {
      ["hvac_mode", "temperature", "fan_mode", "priority", "expected_power_w", "utility", "min_on_seconds", "min_off_seconds"].forEach((key) => {
        const element = form.elements[`load_${index}_${key}`];
        if (element) element.value = load[key] ?? "";
      });
    });
  }
  _collectOptions() {
    ...
    ["source_type", "battery_policy"].forEach((key) => {
      options[key] = form.elements[key].value;
    });
    ["export_reserve_w", "entry_threshold_w", "exit_threshold_w", "battery_full_threshold", "settling_seconds"].forEach((key) => {
      options[key] = Number(form.elements[key].value);
    });
  ...
  _addLoad() {
    const options = this._collectOptions();
    options.loads.push({ entity_id: "", hvac_mode: "dry", temperature: null, fan_mode: null, priority: 100, expected_power_w: null, utility: 1, min_on_seconds: 900, min_off_seconds: 900, enabled: true });
    this._options = options;
    this._render();
  }
  _removeLoad(index) {
    const options = this._collectOptions();
    options.loads.splice(index, 1);
    this._options = options;
    this._render();
  }
  _entityField(key, label, help) {
    return `${this._label(label, help)}<ha-selector data-key="${key}"></ha-selector>`;
  }
  _numberField(key, label, help, min, max, unit) {
    return `${this._label(label, help)}<div class="input-group"><input class="form-control" type="number" id="${key}" name="${key}" min="${min}" ${max === null ? "" : `max="${max}"`}><span class="input-group-text">${unit}</span></div>`;
  }
  _selectField(key, label, help, options) {
    return `${this._label(label, help)}<select class="form-select" id="${key}" name="${key}">${options.map(([value, text]) => `<option value="${value}">${text}</option>`).join("")}</select>`;
  }
  _label(label, help) {
    const id = `tip_${label.replaceAll(/[^a-z0-9]/gi, "_")}`;
    return `<div class="d-flex align-items-center gap-1 mb-1"><label class="form-label mb-0">${this._escape(label)}</label><button class="btn btn-sm btn-link p-0 text-decoration-none" type="button" aria-label="Help: ${this._escape(label)}" data-bs-toggle="tooltip" data-bs-title="${this._escape(help)}">?</button></div>`;
  }
  ...
  _loadNumber(index, key, label, help, min, max, unit, step = "any") {
    return `${this._label(label, help)}<div class="input-group"><input class="form-control" type="number" name="load_${index}_${key}" min="${min}" step="${step}" ${max === null ? "" : `max="${max}"`}><span class="input-group-text">${unit}</span></div>`;
  }
  _loadSelect(index, key, label, help, options) {
    return `${this._label(label, help)}<select class="form-select" name="load_${index}_${key}">${options.map(([value, text]) => `<option value="${value}">${text}</option>`).join("")}</select>`;
  }
  _activateTooltips() {
    this._tooltips = [...this._shadow.querySelectorAll('[data-bs-toggle="tooltip"]')].map((element) => new import_tooltip.default(element));
  }
  _disposeTooltips() {
  ...
  _card(title, value, detail) {
    return `<div class="col-12 col-sm-6 col-xl-3"><div class="card h-100 shadow-sm"><div class="card-body"><div class="text-body-secondary small">${this._escape(title)}</div><div class="fs-4 fw-semibold">${this._escape(value)}</div><div class="small text-body-secondary">${this._escape(detail)}</div></div></div></div>`;
  }
  _loads(loads) {
